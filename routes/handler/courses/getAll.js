...
module.exports = async (req, res) => {
    try {
        ...
        coursesData.data.first_page_url = `${HOTSNAME}/courses?${firstPage}`;
        coursesData.data.last_page_url = `${HOTSNAME}/courses?${lastPage}`;

        if(coursesData.data.next_page_url) {
            const nextPage =  coursesData.data.next_page_url.split('?').pop();
            coursesData.data.next_page_url = `${HOTSNAME}/courses?${nextPage}`;
        }
        
        if(coursesData.data.prev_page_url) {
            const prevPage =  coursesData.data.prev_page_url.split('?').pop();
            coursesData.data.prev_page_url = `${HOTSNAME}/courses?${prevPage}`;
        }
        
        coursesData.data.path = `${HOTSNAME}/courses`;
